...
class BiForwardSample(object):
	...
	def __init__(self, model, checkpoint_dir, batch_size=1, iterations = 100000, masked_track = 8):
		self.model = model
		...
		self.final_backward_state, self.rnn_backward_outputs = model.run_backward_rnn(self.rnn_backward_state, self.backward_input_placeholder)
	...
	def sample(self, n_steps, initial_timeslices=None, condition_dicts=None):
		...
		mask[self.masked_track] = 0
		timeslice_future = [np.array(condition_dicts[i][name]) * mask for name in self.model.condition_shapes for i in range(len(condition_dicts))]
		...
		if initial_timeslices is not None:
			...
			rnn_backward_input = np.stack(backward_input_vecs)
		else:
			...
			rnn_backward_input = self.model.next_backward_rnn_input(timeslice_future, condition_dict, self.masked_track)
			# backward_condition_dict = {} if (condition_dicts is None) else condition_dicts[1:]

			# Create a singleton 'time' dimension
			rnn_forward_input = rnn_forward_input[np.newaxis]
			rnn_backward_input = rnn_backward_input[np.newaxis]
		# Batchify the rnn input and each timeslice in the history
		rnn_forward_input = np.tile(rnn_forward_input, [self.batch_size, 1, 1])
		rnn_backward_input = np.tile(rnn_backward_input, [self.batch_size, 1, 1])
		...
		timeslice_futures = [list(timeslice_future) for i in range(self.batch_size)]

		# Initialize state
		rnn_forward_state = self.sess.run(self.rnn_forward_state)
		rnn_backward_state = self.sess.run(self.rnn_backward_state)
		...
		for i in range(n_steps):
			...
			rnn_forward_state, rnn_backward_state, sample_batch = \
				self.sample_step(rnn_forward_state, rnn_forward_input, rnn_backward_state, rnn_backward_input, condition_dict_batch)
			...
			if i != n_steps - 1:
				rnn_next_backward_inputs = [self.model.next_backward_rnn_input(future, condition_dict, self.masked_track) for future in timeslice_futures]
				rnn_backward_input = np.stack(rnn_next_backward_inputs)[:,np.newaxis]	# newaxis also adds singleton time dimension
	...
	"""
	def sample_step(self, rnn_forward_state, rnn_forward_input, rnn_backward_state, rnn_backward_input, condition_dict):
		...
		feed_dict = { self.forward_input_placeholder: rnn_forward_input, self.rnn_forward_state: rnn_forward_state,
		 			self.backward_input_placeholder: rnn_backward_input, self.rnn_backward_state: rnn_backward_state}
		...
		return next_forward_state, next_backward_state, sample
